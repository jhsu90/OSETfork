...
def peak_det_likelihood(data, fs, params=None):
    ...
    bumps_indexes = bumps_indexes[
        (bumps_indexes >= 0) & (bumps_indexes < sig_len)
    ]

    """
    bumps_indexes same with matlab
    """
    ...
    peak_indexes_padded, _ = refine_peaks_too_close_low_amp(data_filtered_env_padded, bumps_indexes_padded, rpeak_search_half_wlen, env_pk_detect_mode, params["PLOT_DIAGNOSTIC"])
    ...
    """
    peak_indexes same with matlab
    """
    """
    matched filter using average beat shape
    """
    if 'ENHANCE_MATCHED_FILTER' not in params or params['ENHANCE_MATCHED_FILTER'] is None:
    ...
    if params['ENHANCE_MATCHED_FILTER']:
        ...
        data_filtered_enhanced_padded, data_filtered_enhanced_env_padded = signal_specific_matched_filter(
            data_filtered_padded, peak_indexes_padded)

        # Extract the actual data length from the padded data
        data_filtered_enhanced = data_filtered_enhanced_padded[:, left_pad_len:left_pad_len + sig_len]
        data_filtered_enhanced_env = data_filtered_enhanced_env_padded[left_pad_len:left_pad_len + sig_len] # modified to comply with python style slicing

        # Diagnostic plotting
        if params["PLOT_DIAGNOSTIC"]:
            plt.figure()
            plt.plot(data_filtered_padded.mean(axis=0), label='Original Filtered Data')
            plt.plot(data_filtered_enhanced.mean(axis=0), label='Enhanced Filtered Data')
            plt.plot(data_filtered_enhanced_env.mean(axis=0), label='Enhanced Filtered Envelope')
    ...
    if params['REFINE_PEAKS'] and len(peak_indexes) > 1:
        ...
        """
        if 'OMIT_BEATS_MOST_IMPACTED_BY_PRE_PROC' not in params or params[
            'OMIT_BEATS_MOST_IMPACTED_BY_PRE_PROC'] is None:
            params['OMIT_BEATS_MOST_IMPACTED_BY_PRE_PROC'] = True
            if params["verbose"]:
                print(f"params.OMIT_BEATS_MOST_IMPACTED_BY_PRE_PROC = {params['OMIT_BEATS_MOST_IMPACTED_BY_PRE_PROC']}")
            ...
            if params["verbose"]:
                ...
                print(f"pparams.pre_to_post_filter_power_ratio_th = {pparams['pre_to_post_filter_power_ratio_th']}")
        ...
        if params['OMIT_BEATS_MOST_IMPACTED_BY_PRE_PROC']:
            _, peaks_OMIT_BEATS_MOST_IMPACTED_BY_PRE_PROC = refine_peaks_filter_energy_impacted_beats(data_mn_all_channels_padded,
                                                                      data_filtered_mn_all_channels_padded,
                                                                      peak_indexes_padded, pparams)
            peaks_OMIT_BEATS_MOST_IMPACTED_BY_PRE_PROC = peaks_OMIT_BEATS_MOST_IMPACTED_BY_PRE_PROC[left_pad_len:left_pad_len + sig_len]

            peaks_all_method = np.vstack((peaks_all_method, peaks_OMIT_BEATS_MOST_IMPACTED_BY_PRE_PROC))
            refinement_methods.append('OMIT_BEATS_MOST_IMPACTED_BY_PRE_PROC')

            # Set weighting for the refinement method
            ...
            consensus_weights.append(params['weight_OMIT_BEATS_MOST_IMPACTED_BY_PRE_PROC'])

        """
        detect beats with extreme amplitude deviations from other peaks
        (one peak off from matlab result)
        """
        if 'OMIT_LOW_AMP_PEAKS_PRCTL_FRAC' not in params or params['OMIT_LOW_AMP_PEAKS_PRCTL_FRAC'] is None:
            params['OMIT_LOW_AMP_PEAKS_PRCTL_FRAC'] = True
            if params.get('verbose', False):
                print(f"params.OMIT_LOW_AMP_PEAKS_PRCTL_FRAC = {params['OMIT_LOW_AMP_PEAKS_PRCTL_FRAC']}")

        if params['OMIT_LOW_AMP_PEAKS_PRCTL_FRAC']:
            # Setting peak refinement parameters
            pparams = {}
            pparams['percentile'] = 90.0
            ...
            _, peaks_OMIT_LOW_AMP_PEAKS_PRCTL_FRAC = refine_peaks_low_amp_peaks_prctile_fraction(data_filtered_env,
                                                                                              peak_indexes, pparams,
                                                                                              params.get(
                                                                                                  'PLOT_DIAGNOSTIC',
                                                                                                  False))
            peaks_all_method = np.vstack((peaks_all_method, peaks_OMIT_LOW_AMP_PEAKS_PRCTL_FRAC))
        ...
        """
        if 'OMIT_LOW_POWER_BEATS' not in params or params['OMIT_LOW_POWER_BEATS'] is None:
            ...
            if params.get('verbose', False):
                ...
                print(f"   pparams.beat_std_med_frac_th = {params['beat_std_med_frac_th']}")
                print(f"   pparams.max_amp_prctile = {params['max_amp_prctile']}")

        if params['OMIT_LOW_POWER_BEATS']:
            # Assuming refine_peaks_low_power_beats is defined elsewhere
            _, peaks_OMIT_LOW_POWER_BEATS = refine_peaks_low_power_beats(
                data_filtered_mn_all_channels_padded, peak_indexes_padded, pparams['max_amp_prctile'], pparams['beat_std_med_frac_th'],
                params.get('PLOT_DIAGNOSTIC', False)
            )
            peaks_OMIT_LOW_POWER_BEATS = peaks_OMIT_LOW_POWER_BEATS[left_pad_len:left_pad_len + sig_len]

            # Update the master list of peaks and methods
            peaks_all_method = np.vstack((peaks_all_method, peaks_OMIT_LOW_POWER_BEATS))
            refinement_methods.append('OMIT_LOW_POWER_BEATS')

            # Manage the weighting for this specific refinement method
            if 'weight_OMIT_LOW_POWER_BEATS' not in params or params['weight_OMIT_LOW_POWER_BEATS'] is None:
                params['weight_OMIT_LOW_POWER_BEATS'] = 1.0
                if params.get('verbose', False):
                    print(f"params.weight_OMIT_LOW_POWER_BEATS = {params['weight_OMIT_LOW_POWER_BEATS']}")
        ...
        (same)
        """
        if 'OMIT_NEG_CORRCOEF_BEATS' not in params or params['OMIT_NEG_CORRCOEF_BEATS'] is None:
            params['OMIT_NEG_CORRCOEF_BEATS'] = True
            if params.get('verbose', False):
                print(f"params.OMIT_NEG_CORRCOEF_BEATS = {params['OMIT_NEG_CORRCOEF_BEATS']}")

        if params['OMIT_NEG_CORRCOEF_BEATS']:
            # Assuming refine_peaks_waveform_similarity is defined elsewhere
            _, peaks_OMIT_NEG_CORRCOEF_BEATS = refine_peaks_waveform_similarity(
                data_filtered_mn_all_channels_padded, peak_indexes_padded, [], 'NEG-CORR',
                params.get('PLOT_DIAGNOSTIC', False)
            )
            peaks_OMIT_NEG_CORRCOEF_BEATS = peaks_OMIT_NEG_CORRCOEF_BEATS[left_pad_len:left_pad_len + sig_len]

            # Concatenate new peaks to the master list
            peaks_all_method = np.vstack((peaks_all_method, peaks_OMIT_NEG_CORRCOEF_BEATS))
            refinement_methods.append('OMIT_NEG_CORRCOEF_BEATS')

            # Manage the weighting for this refinement method
            if 'weight_OMIT_NEG_CORRCOEF_BEATS' not in params or params['weight_OMIT_NEG_CORRCOEF_BEATS'] is None:
                params['weight_OMIT_NEG_CORRCOEF_BEATS'] = 1.0
                if params.get('verbose', False):
                    print(f"params.weight_OMIT_NEG_CORRCOEF_BEATS = {params['weight_OMIT_NEG_CORRCOEF_BEATS']}")
        ...
        (same)
        """
        if 'OMIT_LOW_CORRCOEF_BEATS' not in params or params['OMIT_LOW_CORRCOEF_BEATS'] is None:
            params['OMIT_LOW_CORRCOEF_BEATS'] = True
            pparams = {}
            pparams['k_sigma'] = 3.0  # Setting a default standard deviation multiplier
            if params.get('verbose', False):
                print(f"params.OMIT_LOW_CORRCOEF_BEATS = {params['OMIT_LOW_CORRCOEF_BEATS']}")
                print(f"   pparams.k_sigma = {params['k_sigma']}")

        if params['OMIT_LOW_CORRCOEF_BEATS']:
            # Assuming refine_peaks_waveform_similarity is defined to handle this functionality
            _, peaks_OMIT_LOW_CORRCOEF_BEATS = refine_peaks_waveform_similarity(
                data_filtered_env_padded, peak_indexes_padded, pparams, 'BEAT-STD', params.get('PLOT_DIAGNOSTIC', False)
            )
            peaks_OMIT_LOW_CORRCOEF_BEATS = peaks_OMIT_LOW_CORRCOEF_BEATS[left_pad_len:left_pad_len + sig_len]

            # Update the master list of peaks and methods
            peaks_all_method = np.vstack((peaks_all_method, peaks_OMIT_LOW_CORRCOEF_BEATS))
            refinement_methods.append('OMIT_LOW_CORRCOEF_BEATS')

            # Manage the weighting for this specific refinement method
            if 'weight_OMIT_LOW_CORRCOEF_BEATS' not in params or params['weight_OMIT_LOW_CORRCOEF_BEATS'] is None:
                params['weight_OMIT_LOW_CORRCOEF_BEATS'] = 1.0
                if params.get('verbose', False):
                    print(f"params.weight_OMIT_LOW_CORRCOEF_BEATS = {params['weight_OMIT_LOW_CORRCOEF_BEATS']}")
        ...
        if 'OMIT_LOW_CORR_BEATS' not in params or params['OMIT_LOW_CORR_BEATS'] is None:
            ...
            if params.get('verbose', False):
                ...
                print(f"   pparams.percentile_fraction = {params['percentile_fraction']}")

        if params['OMIT_LOW_CORR_BEATS']:
            # Assuming refine_peaks_waveform_similarity is defined elsewhere
            _, peaks_OMIT_LOW_CORR_BEATS = refine_peaks_waveform_similarity(
                data_filtered_mn_all_channels_padded, peak_indexes_padded, pparams, 'CORR',
                params.get('PLOT_DIAGNOSTIC', False)
            )
            peaks_OMIT_LOW_CORR_BEATS = peaks_OMIT_LOW_CORR_BEATS[left_pad_len:left_pad_len + sig_len]
            ...
            # Manage the weighting for this specific refinement method
            if 'weight_OMIT_LOW_CORR_BEATS' not in params or params['weight_OMIT_LOW_CORR_BEATS'] is None:
                params['weight_OMIT_LOW_CORR_BEATS'] = 1.0
                if params.get('verbose', False):
                    print(f"params.weight_OMIT_LOW_CORR_BEATS = {params['weight_OMIT_LOW_CORR_BEATS']}")
        ...
        if params['OMIT_LOW_CORRCOEF_BEATS']:
            # Assuming refine_peaks_waveform_similarity is defined elsewhere
            _, peaks_OMIT_LOW_CORRCOEF_BEATS = refine_peaks_waveform_similarity(
                data_filtered_mn_all_channels_padded, peak_indexes_padded, pparams, 'CORRCOEF', params.get('PLOT_DIAGNOSTIC', False)
            )
            peaks_OMIT_LOW_CORRCOEF_BEATS = peaks_OMIT_LOW_CORRCOEF_BEATS[left_pad_len:left_pad_len + sig_len]
        ...
        """
        if 'OMIT_LOW_AMP_PEAKS_PRCTL_ABS' not in params or params['OMIT_LOW_AMP_PEAKS_PRCTL_ABS'] is None:
        ...
        if params['OMIT_LOW_AMP_PEAKS_PRCTL_ABS']:
            _, peaks_OMIT_LOW_AMP_PEAKS_PRCTL_ABS = refine_peaks_low_amp_peaks_prctile(
                data_filtered_env_padded, peak_indexes_padded, 'PRCTILE', pparams['peak_amps_hist_prctile'],
                params.get('PLOT_DIAGNOSTIC', False)
            )
            peaks_OMIT_LOW_AMP_PEAKS_PRCTL_ABS = peaks_OMIT_LOW_AMP_PEAKS_PRCTL_ABS[left_pad_len:left_pad_len + sig_len]

            # Update the master list of peaks and methods
            peaks_all_method = np.vstack((peaks_all_method, peaks_OMIT_LOW_AMP_PEAKS_PRCTL_ABS))
            refinement_methods.append('OMIT_LOW_AMP_PEAKS_PRCTL_ABS')

            # Manage the weighting for this specific refinement method
            if 'weight_OMIT_LOW_AMP_PEAKS_PRCTL_ABS' not in params or params[
                'weight_OMIT_LOW_AMP_PEAKS_PRCTL_ABS'] is None:
        ...
        """
        if 'OMIT_BEAT_HRV_INCR_BEATS' not in params or params['OMIT_BEAT_HRV_INCR_BEATS'] is None:
            params['OMIT_BEAT_HRV_INCR_BEATS'] = True
            if params.get('verbose', False):
                print(f"params.OMIT_BEAT_HRV_INCR_BEATS = {params['OMIT_BEAT_HRV_INCR_BEATS']}")
        ...
        if params['OMIT_BEAT_HRV_INCR_BEATS']:
            ...
            _, peaks_OMIT_BEAT_HRV_INCR_BEATS = refine_peaks_low_snr_beats(
                data_filtered_env_padded, peak_indexes_padded, mmode, params.get('PLOT_DIAGNOSTIC', False)
            )
            peaks_OMIT_BEAT_HRV_INCR_BEATS = peaks_OMIT_BEAT_HRV_INCR_BEATS[left_pad_len:left_pad_len + sig_len]

            peaks_all_method = np.vstack((peaks_all_method, peaks_OMIT_BEAT_HRV_INCR_BEATS))
            refinement_methods.append('OMIT_BEAT_HRV_INCR_BEATS')

            # Manage the weighting for this specific refinement method
            if 'weight_OMIT_BEAT_HRV_INCR_BEATS' not in params or params['weight_OMIT_BEAT_HRV_INCR_BEATS'] is None:
                params['weight_OMIT_BEAT_HRV_INCR_BEATS'] = 3.0
                if params.get('verbose', False):
                    print(f"params.weight_OMIT_BEAT_HRV_INCR_BEATS = {params['weight_OMIT_BEAT_HRV_INCR_BEATS']}")
        ...
        """
        if 'OMIT_BEAT_SNR_REDUC_BEATS' not in params or params['OMIT_BEAT_SNR_REDUC_BEATS'] is None:
            params['OMIT_BEAT_SNR_REDUC_BEATS'] = True
            if params.get('verbose', False):
                print(f"params.OMIT_BEAT_SNR_REDUC_BEATS = {params['OMIT_BEAT_SNR_REDUC_BEATS']}")
        ...
        if params['OMIT_BEAT_SNR_REDUC_BEATS']:
            ...
            # Assuming refine_peaks_low_snr_beats is defined elsewhere
            _, peaks_OMIT_BEAT_SNR_REDUC_BEATS = refine_peaks_low_snr_beats(
                data_filtered_mn_all_channels_padded, peak_indexes_padded, mmode, params.get('PLOT_DIAGNOSTIC', False)
            )
            peaks_OMIT_BEAT_SNR_REDUC_BEATS = peaks_OMIT_BEAT_SNR_REDUC_BEATS[left_pad_len:left_pad_len + sig_len]

            # Update the master list of peaks and methods
            peaks_all_method = np.vstack((peaks_all_method, peaks_OMIT_BEAT_SNR_REDUC_BEATS))
            refinement_methods.append('OMIT_BEAT_SNR_REDUC_BEATS')

            # Manage the weighting for this specific refinement method
            if 'weight_OMIT_BEAT_SNR_REDUC_BEATS' not in params or params['weight_OMIT_BEAT_SNR_REDUC_BEATS'] is None:
                params['weight_OMIT_BEAT_SNR_REDUC_BEATS'] = 1.0
                if params.get('verbose', False):
                    print(f"params.weight_OMIT_BEAT_SNR_REDUC_BEATS = {params['weight_OMIT_BEAT_SNR_REDUC_BEATS']}")
        ...
        if 'OMIT_HIGH_STD_AMP' not in params or params['OMIT_HIGH_STD_AMP'] is None:
            ...
            pparams = {}
            pparams['k_sigma'] = 4.0  # Standard deviation multiplier
            if params.get('verbose', False):
                print(f"   pparams.k_sigma = {params['k_sigma']}")

        if params['OMIT_HIGH_STD_AMP']:
            # Assuming refine_peaks_high_amp_std is defined elsewhere
            _, peaks_OMIT_HIGH_STD_AMP = refine_peaks_high_amp_std(
                data_filtered_env_padded, peak_indexes_padded, pparams['k_sigma'], params.get('PLOT_DIAGNOSTIC', False)
            )
            peaks_OMIT_HIGH_STD_AMP = peaks_OMIT_HIGH_STD_AMP[left_pad_len:left_pad_len + sig_len]
            ...
            # Manage the weighting for this specific refinement method
            if 'weight_OMIT_HIGH_STD_AMP' not in params or params['weight_OMIT_HIGH_STD_AMP'] is None:
                params['weight_OMIT_HIGH_STD_AMP'] = 1.0
                if params.get('verbose', False):
                    print(f"params.weight_OMIT_HIGH_STD_AMP = {params['weight_OMIT_HIGH_STD_AMP']}")
        ...
        if 'NUM_VOTES_TO_KEEP_PEAK' in params and 'CONSENSUS_WEIGHTS_THRESHOLD' in params:
            if params.get('NUM_VOTES_TO_KEEP_PEAK') is not None and params.get(
                    'CONSENSUS_WEIGHTS_THRESHOLD') is not None:
                print(
        ...
        if 'NUM_VOTES_TO_KEEP_PEAK' not in params or params['NUM_VOTES_TO_KEEP_PEAK'] is None:
            if 'CONSENSUS_WEIGHTS_THRESHOLD' not in params or params['CONSENSUS_WEIGHTS_THRESHOLD'] is None:
        ...
        # Merge peaks based on the set thresholds
        if 'CONSENSUS_WEIGHTS_THRESHOLD' in params and params['CONSENSUS_WEIGHTS_THRESHOLD'] is not None:
            if not (0 <= params['CONSENSUS_WEIGHTS_THRESHOLD'] <= 1):
                raise ValueError('params.CONSENSUS_WEIGHTS_THRESHOLD must be between 0 and 1')
            peaks_weighted_average = np.sum(np.dot(np.diag(consensus_weights), peaks_all_method), 0) / sum(consensus_weights)
            peak_indexes_consensus = np.where(peaks_weighted_average >= params['CONSENSUS_WEIGHTS_THRESHOLD'])[0]
        elif 'NUM_VOTES_TO_KEEP_PEAK' in params and params['NUM_VOTES_TO_KEEP_PEAK'] is not None:
            if params['NUM_VOTES_TO_KEEP_PEAK'] > num_peak_refinement_algorithms:
                raise ValueError('Number of required votes to keep a peak exceeds the number of voting algorithms')
            peaks_consensus = np.sum(np.diag(peaks_all_method), axis=0)
            peak_indexes_consensus = np.where(peaks_consensus >= params['NUM_VOTES_TO_KEEP_PEAK'])
    ...
    # Prepare for likelihood calculations
    ...
    for kk in range(num_peak_refinement_algorithms):
        ...
        qrs_likelihoods[kk, :] = peak_surrounding_likelihood(sig_len, peak_indices, fs, 'GAUSSIAN',
                                                             params['max_likelihood_span'], params['likelihood_sigma'])

    # Aggregate likelihoods using consensus weights
    qrs_likelihood = np.sum(np.diag(consensus_weights) @ qrs_likelihoods, axis=0) / np.sum(consensus_weights)
    ...
    if params.get('PLOT_DIAGNOSTIC', False):
        ...
        # Highlighting different types of detected peaks
        plt.plot(tt[bumps_indexes], data_filtered_env[bumps_indexes], 'go', label='bumps_indexes')
        plt.plot(tt[peak_indexes], data_filtered_env[peak_indexes], 'rx', markersize=18, label='peak_indexes')
        plt.plot(tt[peak_indexes_consensus], data_filtered_env[peak_indexes_consensus], 'ko', markersize=24,
    ...
    if params['RETURN_SIGNAL_PEAKS']:
        ...
        if 'envelope_to_peak_search_wlen' not in params:
            ...
            if params.get('verbose', False):
                print(f"params.envelope_to_peak_search_wlen = {params['envelope_to_peak_search_wlen']}")

        # Calculate window length in samples
        envelope_to_peak_search_wlen = int(fs * params['envelope_to_peak_search_wlen'] // 2)

        # Process for initial peak indexes
        peak_likelihood_boxes = peak_surrounding_likelihood(sig_len, peak_indexes, fs, 'BOX',
                                                            params['envelope_to_peak_search_wlen'], [])
        peak_indexes, _ = find_closest_peaks(data * np.tile(peak_likelihood_boxes, (data_filtered.shape[0], 1)),
                                          peak_indexes, envelope_to_peak_search_wlen, params['PEAK_SIGN'],
                                          params['PLOT_DIAGNOSTIC'])

        # Process for consensus peak indexes
        peak_likelihood_boxes = peak_surrounding_likelihood(sig_len, peak_indexes_consensus, fs, 'BOX',
                                                            params['envelope_to_peak_search_wlen'], [])
        peak_indexes_consensus, _ = find_closest_peaks(data * np.tile(peak_likelihood_boxes, (data_filtered.shape[0], 1)),
                                                    peak_indexes_consensus, envelope_to_peak_search_wlen,
                                                    params['PEAK_SIGN'], params['PLOT_DIAGNOSTIC'])
    """
    post-extraction peak refinement based on likelihoods
    """
    if 'POST_EXT_LIKELIHOOD_BASED_IMPROVEMENT' not in params or params['POST_EXT_LIKELIHOOD_BASED_IMPROVEMENT'] is None:
        params['POST_EXT_LIKELIHOOD_BASED_IMPROVEMENT'] = False
        if params.get('verbose', False):
            print(f"params.POST_EXT_LIKELIHOOD_BASED_IMPROVEMENT = {params['POST_EXT_LIKELIHOOD_BASED_IMPROVEMENT']}")
    # If the likelihood-based improvement is enabled, perform the refinement
    if params['POST_EXT_LIKELIHOOD_BASED_IMPROVEMENT']:
        likelihood_threshold = 0.4
        if params['verbose']:
            print(f"   likelihood_threshold = {likelihood_threshold}")

        # Assume refine_peaks_low_likelihood is a function defined to refine peaks based on likelihood
        peak_indexes_consensus = refine_peaks_low_likelihood(data_filtered, peak_indexes_consensus, qrs_likelihood,
                                                             likelihood_threshold, rpeak_search_half_wlen,
                                                             params.get('PLOT_DIAGNOSTIC', False))
    ...
    if params.get('PLOT_RESULTS', False):
        ...
        # Set up the plot
        plt.figure(figsize=(15, 8))  # Adjust the size to match MATLAB's 'normalized' and 'outerposition'

        # Scatter plot for QRS likelihood (color-coded)
        colors = np.array([0.5, 0.5, 0.5]) + qrs_likelihood * np.array(
            [0, 0, 0.5])  # Adjust color calculation as needed
        plt.scatter(tt, data, c=colors, s=100, edgecolors='none', label='QRS likelihood (color-coded from gray to red)')
        ...
        # Highlight bumps indexes
        plt.plot(tt[bumps_indexes], data_filtered_env[bumps_indexes], 'g.', markersize=14, label='Bumps indexes')

        # Different markers for different refinement methods
        all_marks = ['o', '+', '*', '.', 'x', 's', 'd', '>', 'v', '<', '^', 'p', 'h']
        for ll, method in enumerate(refinement_methods):
            pk_indx = np.where(peaks_all_method[ll, :])[0]
            if len(pk_indx) > 0:
                plt.plot(tt[pk_indx], data_filtered_env[pk_indx], all_marks[ll % len(all_marks)], markersize=ll + 10,
                         label=method)

        # Detected R-peaks
        plt.plot(tt[peak_indexes], data[peak_indexes], 'co', markersize=16, label='Detected R-peaks')
        # Corrected R-peaks
        plt.plot(tt[peak_indexes_consensus], data[peak_indexes_consensus], 'ko', markerfacecolor='r', markersize=20,
